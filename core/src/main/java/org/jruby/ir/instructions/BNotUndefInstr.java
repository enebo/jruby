...
public class BNotUndefInstr extends OneOperandBranchInstr  implements FixedArityInstr {
    public BNotUndefInstr(Label jmpTarget, Operand v) {
        super(Operation.B_UNDEF, jmpTarget, v);
    }

    @Override
    public Instr clone(CloneInfo ii) {
        return new BNotUndefInstr(ii.getRenamedLabel(getJumpTarget()), getArg1().cloneForInlining(ii));
    }
    ...
    public static BNotUndefInstr decode(IRReaderDecoder d) {
        return new BNotUndefInstr(d.decodeLabel(), d.decodeOperand());
    }
